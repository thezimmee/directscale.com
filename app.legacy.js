...
(function () {
  ...
  var getSiblings = (function (el, selector) {
  ...
  Toggles.prototype.open = function (event) {
    ...
    setTimeout(function () {
      _this.dom.toggle.classList.add(_this.config.activeClass);

      _this.dom.content.classList.add(_this.config.activeClass);

      _this.active = true;
    }, 20); // Remove animate in class.

    setTimeout(function () {
      _this.dom.toggle.classList.remove(_this.config.animateInClass);

      _this.dom.content.classList.remove(_this.config.animateInClass);
  ...
  Toggles.prototype.close = function (event) {
    ...
    setTimeout(function () {
      _this2.dom.toggle.classList.remove(_this2.config.activeClass);

      _this2.dom.content.classList.remove(_this2.config.activeClass);

      _this2.active = false;
      if (_this2.group && _this2.group.active === _this2) _this2.group.active = null;
    }, 20); // Remove animate out class.

    setTimeout(function () {
      _this2.dom.toggle.classList.remove(_this2.config.animateOutClass);

      _this2.dom.content.classList.remove(_this2.config.animateOutClass);
  ...
  function Toggles(element) {
    ...
    if (this.dom.closeButtons && this.dom.closeButtons.length) {
      this.dom.closeButtons.forEach(function (button) {
        return button.addEventListener('click', _this3.close.bind(_this3));
      });
    }

    if (this.dom.openButtons && this.dom.openButtons.length) {
      this.dom.openButtons.forEach(function (button) {
        return button.addEventListener('click', _this3.open.bind(_this3));
      });
    }
  }
  ...
  DirtyInput.init = function () {
    ...
    return document.querySelectorAll(selector).forEach(function (i) {
      return new DirtyInput(i);
    });
  };
